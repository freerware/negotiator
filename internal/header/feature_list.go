...
func NewFeatureList(features []string) FeatureList {
	var list []FeatureListElement
	for _, feature := range features {
		var le FeatureListElement
		if regexpPredicateBag.MatchString(feature) {
			le = newPredicateBagListElement(feature)
		} else {
			le = newPredicateListElement(feature)
		}
		list = append(list, le)
	}
